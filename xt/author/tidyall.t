...
#pod =for :stopwords tidyall perltidy perlcritic config CPAN perl
#pod
#pod =cut
...
#pod     prove -lr xt/author/tidyall.t
#pod
...
#pod
#pod Runs L<tidyall|tidyall> as specified by the F<tidyall.ini> file. This code is
#pod based on the test auto-generated by
#pod L<Dist::Zilla::Plugin::Test::TidyAll|Dist::Zilla::Plugin::Test::TidyAll>.
#pod Essentially just a wrapper around calling
#pod L<Test::Code::TidyAll::tidyall_ok()|Test::Code::TidyAll/DESCRIPTION>.
#pod
#pod The L<tidyall|tidyall> program wraps multiple code checkers, cleaners and
#pod testers in one interface. This test uses that interface to insure that
#pod L<tidyall|tidyall> has been run, i.e. it runs L<tidyall|tidyall> in "test" mode.
#pod As configured by the F<tidyall.ini> file, this runs two programs,
#pod L<perltidy|perltidy> and L<perlcritic|perlcritic>:
#pod
#pod =for :list  L<perltidy|perltidy> - All perl files using the config file
#pod F<pertidy.ini>.
#pod
#pod =for :list  L<perlcritic|perlcritic> - All F<*.pm> files under F<lib/>
#pod (including subdirectories) using the config file F<perlcritic.ini>.
#pod
#pod =for :list  L<perlcritic|perlcritic> - All F<*.t> files under F<t/> and F<xt/>
#pod (including subdirectories) using the config file F<perlcritic_test.ini>.
#pod
#pod The L<perltidy|perltidy> and L<perlcritic|perlcritic> programs are both, in
#pod general, quality checkers and as such are run as author tests. As with all
#pod author tests, this test and the resources it needs (i.e. the config files) have
#pod to be part of the distributed CPAN directory, even though they are not for the
#pod user.
#pod
#pod The perl critic tests are extensive, but carefully customized via their config
#pod file to give, on average, few false positive complaints. (Start Rant) It is
#pod unusual for Perl distributions to run critic tests on the *.t files. This is
#pod evidence that Perl authors hold tests to be exempt from good coding practices,
#pod which implies that it is not important for tests to be correct or to be written
#pod well... "My time. It's Precious. Tests-es, we hates them." (End Rant).
#pod
#pod =cut
...
#pod =head1 TESTS
#pod
#pod =head2 Calling Test::Code::TidyAll::tidyall_ok()
#pod
#pod Loads the tidyall config file and uses it to run a bunch of other tests.
#pod
#pod =cut
...
=for :stopwords Stuart R. Jefferys tidyall perltidy perlcritic config CPAN perl

=head1 NAME

xt/author/tidyall.t - Check that tidyall runs without error
